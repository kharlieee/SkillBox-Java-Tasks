...
public class RoadController {
    ...
    public static void main(String[] args) {
        ...
        int carsCount = scanner.nextInt(); //Обьявление переменной //
        ...
        for (int i = 0; i < carsCount; i++) {
            ...
            int price = calculatePrice(car); // Обьявление переменной//
            if (price == -1) {
                continue;
            }
    ...
    private static int calculatePrice(Car car) {
        int carHeight = car.height;// Обьявление переменной//
        int price = 0;// Обьявление переменной //
        if (carHeight > controllerMaxHeight) {
            blockWay("высота вашего ТС превышает высоту пропускного пункта!");
            return -1;
        } else if (carHeight > passengerCarMaxHeight) {
            double weight = car.weight; // Обьявление переменной //
            //Грузовой автомобиль
